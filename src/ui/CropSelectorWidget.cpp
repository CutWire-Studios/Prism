...
std::array<QRectF, 4> CropSelectorWidget::handles(const QRectF &crop) const {
    return { QRectF(crop.left()  - HS2, crop.top()    - HS2, HS, HS),
             QRectF(crop.right() - HS2, crop.top()    - HS2, HS, HS),
             QRectF(crop.right() - HS2, crop.bottom() - HS2, HS, HS),
             QRectF(crop.left()  - HS2, crop.bottom() - HS2, HS, HS) };
}
...
void CropSelectorWidget::commitDrag(const QRectF &crop) {
    ...
    m_cx = static_cast<float>((crop.left() - fr.left()) / fr.width());
    m_cy = static_cast<float>((crop.top()  - fr.top())  / fr.height());
    m_cw = static_cast<float>(crop.width()  / fr.width());
    m_ch = static_cast<float>(crop.height()  / fr.height());
...
void CropSelectorWidget::paintEvent(QPaintEvent *) {
    ...
    p.setPen(Qt::NoPen);
    for (const QPointF &corner : {cr.topLeft(), cr.topRight(), cr.bottomLeft(), cr.bottomRight()})
